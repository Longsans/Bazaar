namespace Bazaar.Basket.IntegrationEvents.Events;
...
public record BuyerCheckoutAcceptedIntegrationEvent : IntegrationEvent
{
        ...
        string buyerId, BuyerBasket basket)
    {
        ...
        Basket = new CheckoutEventBasket(
            basket.BuyerId,
            basket.Items.Select(item => new CheckoutEventBasketItem(item.ProductId, item.ProductName, item.UnitPrice, item.Quantity)));
    }
}
